...
using DM.AbpZeroTemplate.Authorization.Impersonation;
...
using DM.AbpZeroTemplate.Security;
using DM.AbpZeroTemplate.Web.Controllers.Results;
using DM.AbpZeroTemplate.Web.Models.Account;
...
namespace DM.AbpZeroTemplate.Web.Controllers
{
    public class AccountController : AbpZeroTemplateControllerBase
    {
        ...
        public virtual async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            if (loginResult.User.ShouldChangePasswordOnNextLogin)
            {
                ...
                return Json(new MvcAjaxResponse
                {
                    TargetUrl = Url.Action(
                        ...
                        {
                            TenantId = Security.SimpleStringCipher.Encrypt(loginResult.Tenant.Id.ToString()),
            ...
            int? tenantId = loginResult.Tenant == null ? (int?)null : loginResult.Tenant.Id;
        ...
        public virtual async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            var tenantId = Convert.ToInt32(Security.SimpleStringCipher.Decrypt(model.TenantId));
            using (UnitOfWorkManager.Current.SetTenantId(tenantId))
            {
                _unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant);
        ...
        public virtual async Task<JsonResult> Impersonate(ImpersonateModel model)
        {
            ...
            using (UnitOfWorkManager.Current.SetTenantId(model.TenantId))
            {
        ...
        {
            using (UnitOfWorkManager.Current.SetTenantId(AbpSession.ImpersonatorTenantId))
            {
                if (!AbpSession.ImpersonatorUserId.HasValue)
                {
